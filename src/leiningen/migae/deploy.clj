...
(defn deploy
  "upload app the the prod servers - 'lein migae :deploy'"
  [project & args]
  (println "updating to prod cloud")
  (eval/eval-in-project
   project ;; AppCfg will barf if run from the jar in ~/.m2
   ;; set appengine.sdk.root to prevent this
   (let [appengine-sdk (:sdk (:migae project))
         appengine-sdk (cond (nil? appengine-sdk)
                             (if-let [from-env (System/getenv "APPENGINE_HOME")]
                               from-env
                               (abort
                                (str "deploy "
                                     (str "no App Engine SDK specified in project.clj"
                                          "(set :migae {:sdk \"path/to/sdk\""
